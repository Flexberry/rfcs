...
Основная идея - рядом с прикладным приложением разворачивается служба адаптер, которая, при минимальном изменение внутри прикладного проекта-источника, собирает данные об изменениях объектов внутри прикладной системы. 
...
Для изменения мастеров используются сабмапперы, настройки их использования прописываются для каждого объекта отдельно. Детейлам соответствуют отдельные мапперы, где агрегатор выступает в роли мастера. Для преобразования из одного объекта в два используются два разных маппера с одним объектом-источником и разными объектами-приемниками.
...
3. В описании шины добавить ссылку на этот адаптер.


## Недостатки

> Предложенный подход не сможет удовлетворить необходимые потребности для решения проблем интеграции и синхронизации, что вынудить разработчиков создавать узкоспециализированные приложения, годные для работы только внутри их прикладных проектов.
...
> Проблемы:
* Связанные в цепочку изменения - порядок сообщений
* Коллии первичных ключей, хранение ключей других источников?
* Удаление объектов во многих системых
* Запрос по ключу
